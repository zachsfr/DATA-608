...
by_country <- cleaned_up_data  %>% group_by(Country,decade)  %>% summarise(avg_per_country= round(  mean(AverageTemperature),2 ))   %>% mutate( pct_change =  round((avg_per_country- lag(avg_per_country))/ lag(avg_per_country)  * 100  ,2),previous_decade = lag(avg_per_country)  )  %>%
  replace(is.na(.), 0)  %>% ungroup() %>% filter(decade>1840)
...
```{r}
by_country  %>% filter(str_detect(Country ,"Ec"))
```
